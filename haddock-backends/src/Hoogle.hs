...
import GHC
  ( ConDecl
    ...
    HsConDeclGADTDetails (PrefixConGADT, RecConGADT),
    HsConDetails (InfixCon, PrefixCon, RecCon),
    HsDataDefn (dd_cons, dd_derivs),
    HsDecl (ForD, SigD, TyClD),
    HsSigType (HsSig, XHsSigType),
...
import GHC.Unit.State (UnitState, Version (versionBranch))
...
-- uses ppDocumentation, ppExport, ppInstance
...
-- this function should be encapsulated.
...
-- pretty print signature with doc
      ...
      con_sig_ty = HsSig noExtField outer_bndrs theta_ty
        where
          ...
          tau_ty = foldr mkFunTy res_ty $
            case args of
              PrefixConGADT pos_args -> map hsScaledThing pos_args
